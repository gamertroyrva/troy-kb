...
*(For rungs 2+, swap "1 row" → "the next 3 / 6 / 10... currently-unstarted rows," the Rung value in Audit Log entries--Troy sees 4 places to update for rungs 2+which is fine, and update the STOP-CAPS clause per the table in §1.)*
...
5. Check `/usage` reading from before/after this run — record the delta.
