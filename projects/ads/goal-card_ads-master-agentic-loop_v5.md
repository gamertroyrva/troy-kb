...
*This is v5 of this Goal Card, produced after Rung 1C (DONE WHEN met — Venue Number 9, burnley — zero business-analyst escalations, and the clearest measurement-plan read yet: fewer turns, less wall-clock, and lower usage-percentage deltas than Rung 1B, all three metrics agreeing for the first time, strong evidence the v4 hooks worked). This version closes the `dump-audit-log` reporting gap (Issues Log Entry 19) and, more substantially, specifies for the first time how multi-row rungs actually work — worker invoked fresh per row, qa verifying immediately after each row rather than once at the end of the whole rung, and a hard stop on any qa failure. That question had never been made explicit and was invisible through Rungs 1/1B/1C purely because N=1 made every interpretation identical — Rung 2 (3 rows) would have been the first rung to actually expose the ambiguity live. Supersedes v4.*

---

## What changed in v5 (read this first if you read v4)
...
2. **For each of the N target rows, in order:**
   ...
   3. If qa fails either check: **hard stop the entire rung immediately.** Do not proceed to any further rows, even if this was not the last of the N target rows. Report exactly what completed, what failed, and why.
...
**STOP-CAPS** *(rung-specific — table below; extend rightward as data comes in. Caps apply cumulatively across a rung's entire per-row loop, not per individual row.)*
...
reached, OR if 2 hours of wall-clock time elapse - whichever comes first. These
caps apply cumulatively across all 3 rows, not per row. If stopped, report exactly
what was completed, what remains, and why it stopped.
