# Per-Participant Drop Zone Lists — Rules Spec (Block 09)

*This document contains the business rules for the Drop Zone Lists chore. Companion to `trade-stats-rules-spec.md` (Block 07) and `cash-consolidation-rules-spec.md` (Block 08). All three draw from the Gives Receives Trade Log for a given math trade. Block numbers 07–09 intentionally leave room for other blocks in between.*

---

## Purpose

Take the Gives Receives Trade Log (also used for Trade Stats and Cash Consolidation) and produce a trimmed, print-and-cut set of personal "drop zone" labels — one per eligible participant — showing only what they're set to receive at physical swap for that math trade.

---

## Input

**The Gives Receives Trade Log** — the per-participant gives/receives portion extracted from the OLWLG final results webpage for a given math trade. Header line per participant (stats + optional location), followed by numbered `-- N. gives ...` and `++ N. receives ...` lines. Full definition of how this document comes into existence lives in `trade-stats-rules-spec.md` (Block 07).
...
The participant's header stats line (e.g. "(N trades of M, P%)") is carried over unchanged from the Gives Receives Trade Log. It reflects the participant's *original* full math trade activity (gives + receives + cash), not a recalculated count of the trimmed receives-only list shown beneath it. Do not recompute this figure.

---

## Rule 7 — Header Styling

The participant's username, appearing above their trimmed receives list, is formatted large, bold, and blue — sized for legibility from a short physical distance at the swap table.
...
A single document, one section per eligible participant (Rule 4), each formatted per Rules 5–7, in the same order as the Gives Receives Trade Log (alphabetical by handle). This is an intermediate deliverable — final print-ready formatting still requires the Rule 8 manual pass before it's cut into physical drop-zone slips.
