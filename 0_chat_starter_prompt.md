


Every new chat I start is a continuation of my AI learning journey which started on April 26, including this chat.
THE CUSTOM PART
Please load these three files into context including the AI Journal Journey Google sheet, and then we can get started.


Every new chat I start is a continuation of my AI learning journey which started on April 26, including this chat.
This chat is for deciding what to do next, and then doing it.
Please load these three files into context including the AI Journal Journey Google sheet, and then we can get started on the deciding of what to do next.



Every new chat I start is a continuation of my AI learning journey which started on April 26, including this chat.
Between Friday and Saturday, I had a legendary chat with you about Andrew's Driving Service.  There's tons of context in that chat, but the context window is exhausted.  I need to get back going and then get things into a great state before possibly putting this work down for a bit.
Please load these three files into context including the AI Journal Journey Google sheet, and then we can get started.

Every new chat I start is a continuation of my AI learning journey which started on April 26, including this chat.
This session is for working on item currently listed as item 12 in my backlog.	MisoCon26 Attendee Tracker	Miso Association Project	Auto-fill Google Sheet with each addition attendee registration	This provides a return to agent development (kinda,close enough), and satisfies an actual need expressed to me.  This session will also test the context jumpstart process whereby I provide you an MD file gracefully saved from a prior session to enable hydraulic launch on a resumed task.  I'll find out how well this works.  Yay.
Please load these three files into context including the AI Journal Journey Google sheet, and then I'll provide you one more MD file with the details of what I'm restarting.

Every new chat I start is a continuation of my AI learning journey which started on April 26, including this chat.
There's an automation task I'd like to discuss.  Due to time constraints, might not get to build it this morning.
Please load these three files into context including the AI Journal Journey Google sheet, and then we can get started.

Every new chat I start is a continuation of my AI learning journey which started on April 26, including this chat.
this could be a quick one.  I just wanna tell you about the AI Daily podcast from yesterday!
Please load these three files into context including the AI Journal Journey Google sheet, and then we can get started.

Every new chat I start is a continuation of my AI learning journey which started on April 26, including this chat.
We start today with a fun one.  A comment to a LinkedIn Post.  AI Learning meet engaging with others in public discorse.  What could go wrong?
Please load these three files into context including the AI Journal Journey Google sheet, and then we can get started.

Every new chat I start is a continuation of my AI learning journey which started on April 26, including this chat.
Today I want to invest in Obsidian.  I've learned of it from Rubin.  I find the value proposition compelling.  That said, like most things there's the prediscussion about what I think I want to get out of it, etc., etc.  I don't want to spend boatloads of time on it.  80/20 rule should suffice.  
Please load these three files into context including the AI Journal Journey Google sheet, and then we can get started.